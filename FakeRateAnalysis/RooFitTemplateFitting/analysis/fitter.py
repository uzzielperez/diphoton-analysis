...
infile = TFile("./jethtvanillaplots/fakeRatePlots.root","read")

plotorigEB = infile.Get("fakeRateEB_chIso9To14")
plotEB = plotorigEB.Clone()

plotorigEE = infile.Get("fakeRateEE_chIso9To14")
plotEEclone = plotorigEE.Clone()
plotEE = TGraphAsymmErrorsToTH1D(plotEEclone,"plotEE")

EBfunc = TF1("EBfunc","[0]+[1]/(x^[2])",0.,600.)
EBfunc.SetParameter(0,0.2)
EBfunc.SetParameter(1,1.)
EBfunc.SetParameter(2,1.)
...
print "********EE FIT************"
EEfuncHigh = TF1("EEfuncHigh","pol1",170.,600.)
resHigh = plotEE.Fit(EEfuncHigh,"SR")
EEfuncLow = TF1("EEfuncLow","[0]+[1]*(x-175)+[2]*(x-175)^2",45.,180.)
EEfuncLow.FixParameter(0,EEfuncHigh.Eval(175.)) # high pT linear fit evaluated at 175
