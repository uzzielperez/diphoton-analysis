...
std::pair<double,double> rooFitClosureTest(TString sample, TString templateVariable, TString ptBin, TString etaBin, std::pair<double,double> sideband, int denomBin, TString era, int pvCutLow, int pvCutHigh)
{
  ...
  historealmcfile = TFile::Open(mc_filename);
  ...
  if (templateVariable == "sieie") {
    ...
    // templateVariableMax = 0.08;
    templateVariableMax = 0.1; // FIXME
    //(EE only max) // Look at actual histograms. No point going past where we have data.
                              // For endcap, what should be the value?
                              // Limits for the value for the normalization..
  }
  ...
  //? Are these the right min and max values for
  // template normalization

  double template_fit_variable_cut;
  if (templateVariable == "sieie") {
    // using cuts for unsaturated photons
    etaBin.Contains("EB") ? template_fit_variable_cut = 0.0105 : template_fit_variable_cut = 0.028; // C
  }
  else { // otherwise assume "chIso"
    etaBin.Contains("EB") ? template_fit_variable_cut = 5.0 : template_fit_variable_cut = 5.0;
  }
  template_fit_variable.setRange("sigrange",0.0,template_fit_variable_cut); //FIXME
  // Need to fit over the whole range...
  // Are we fitting over the whole range or not?

  RooDataHist faketemplate("faketemplate","fake template",template_fit_variable,hfakeTemplate); // x-axis template_fit_variable: sieie
  RooHistPdf fakepdf("fakepdf","test hist fake pdf",template_fit_variable,faketemplate); // DataHist input to the pdf
  ...
  RooExtendPdf extpdfsig("Signal","extpdfsig",realpdf,signum,"sigrange"); // how this is working?
  RooExtendPdf extpdffake("Background","extpdffake",fakepdf,fakenum,"sigrange"); // are they restricted to fitting within sigrange or is this just for reporting?
  ...
  cout << "CHI2 FIT RES = " << fitres << endl; // get these numbers for the fits that look reasonable by eye

  if (postFix == "_chIso5To10"){
    ofstream chisqres;
    chisqres.open ("chisqres.txt", ios::app);
    chisqres << etaBin << " pt: " << ptBin  <<  postFix  <<" CHI2 FIT RES = " << fitres << endl;
    chisqres.close();
  }
  ...
  TString fitResString = TString::Format("%f", fitres);
  ...
  t_label->     DrawLatexNDC(0.55,0.45, "#chi^{2}: " + fitResString);
  ...
  gPad->SetLogy();
  canvas->Print( TString("plotsEEDebug/fakeRatePlot_") + sample + "_" + era + "_" +  etaBin + TString("_pT") + ptBin + postFix + TString(".pdf") );
