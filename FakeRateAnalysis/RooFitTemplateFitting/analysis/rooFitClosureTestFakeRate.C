...
std::pair<double,double> rooFitFakeRateProducer(TFile *realtemp_inputfile, TFile *faketemp_inputfile, TString sample, TString templateVariable, TString ptBin, TString etaBin, std::pair<double,double> sideband, int denomBin, TString era, int pvCutLow, int pvCutHigh)
{
  ...
  TString basefilename("root://cmseos.fnal.gov//store/user/cuperez");
  ...
  //if (sample == "GGJets")    data_filename = TString::Format("GGJets_fakerate_2018_pV%d-%d_newDenomDef.root", pvCutLow, pvCutHigh);
  if (sample == "GGJets")    data_filename = basefilename + "/GGJets_fakerate_closuretest_2018_pV0-200_newDenomDef.root";
  //if (sample == "GGJets")    data_filename = TString::Format("diphoton_fake_rate_real_templates_all_GGJets_GJets_"+ cmssw_version[era] + "_" + pvCut + "_MiniAOD_histograms.root");
  ...
  if (templateVariable == "sieie") templateVariableTitle = "#sigma_{i#etai#eta}";
  else if (templateVariable == "chIso") templateVariableTitle = "Iso_{Ch}";
