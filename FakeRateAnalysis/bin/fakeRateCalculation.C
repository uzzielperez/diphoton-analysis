...
int main(int argc, char *argv[])
{
  ...
  if(argc < 3) {
    ...
    sample = argv[1];
    if (sample != "jetht" && sample != "doublemuon" && sample != "mc" && sample != "mc_QCD" && sample != "mc_GJets" && sample != "mc_GGJets" && sample !="GGJets") {
      std::cout << "Choose sample: jetht, doublemuon, mc, mc_QCD, mc_GJets, mc_GGJets\n" << std::endl;
      return -1;
    }
  ...
  // if (sample == "GGJets")    input_filename = TString::Format("GGJets_fakerate_2018_pV%d-%d_newDenomDef.root", pvCutLow, pvCutHigh);
