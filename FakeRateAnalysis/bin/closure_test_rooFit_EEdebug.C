...
#include "diphoton-analysis/FakeRateAnalysis/RooFitTemplateFitting/analysis/rooFitClosureTestEEDebug.C"
...
int main(int argc, char *argv[])
{
  ...
  //std::vector<int> ptBinArray({90, 110});
  ...
  TFile outfile("fakeRatePlots_" + sample + "_" + era + pvCut + "EEDebug.root","recreate");
  ...
  TFile outfile2("fakeRatePlots_" + sample + "_" + era + pvCut + "EEDebug.root","update");
  outfile2.cd();
  denomvsptEB->Write();
  denomvsptEE->Write();
  ...
  for (unsigned int j = 0; j < sidebandsEB.size(); j++) {
    ...
    c.SaveAs("plotsEEDebug/fake_rate_" + sample + "_" + era + "_EB"+postFix+ pvCut + ".pdf");
  }
  ...
  for (unsigned int j = 0; j < sidebandsEE.size(); j++) {
    ...
    c.SaveAs("plotsEEDebug/fake_rate_" + sample + "_" + era + "_EE"+postFix+ pvCut + ".pdf");
  }
