...
## Merge ntuples
Merge ntuples from scripts directory (modify as needed):  
``` ./hadd_script.sh ```  
...
For a single binned MC sample run from the analysis directory:  
```
...
If desired, outputted histograms can be combined by running from the analysis directory:
```
