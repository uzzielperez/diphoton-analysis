...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (int i = 30; i<140; i=i+20) {
     TH1D *h = new TH1D(Form("sieieEB_mcFake_pt%dTo%d",i,i+20),"sigmaIetaIetaEB",200,0.,0.1);
     h->Sumw2();
     sigmaIetaIetaEB_pt20.push_back(h);
   }
   vector<TH1D*> sigmaIetaIetaEB_pt50;  
   for (int i = 150; i<650; i=i+50) {
     TH1D *h = new TH1D(Form("sieieEB_mcFake_pt%dTo%d",i,i+50),"sigmaIetaIetaEB",200,0.,0.1);
     h->Sumw2();
   ...
   for (int i = 30; i<140; i=i+20) {
     TH1D *h = new TH1D(Form("sieieEE_mcFake_pt%dTo%d",i,i+20),"sigmaIetaIetaEE",200,0.,0.1);
     h->Sumw2();
     sigmaIetaIetaEE_pt20.push_back(h);
   }
   vector<TH1D*> sigmaIetaIetaEE_pt50;  
   for (int i = 150; i<650; i=i+50) {
     TH1D *h = new TH1D(Form("sieieEE_mcFake_pt%dTo%d",i,i+50),"sigmaIetaIetaEE",200,0.,0.1);
     h->Sumw2();
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     // only select photon matched to gluons or quarks (no photon matches are stored)
     //if (PhotonGenMatch_pdgId != 21 && fabs(PhotonGenMatch_pdgId) > 7) continue;
   ...
   TString filename;
   filename = TString::Format("diphoton_fakeRate_QCD_Pt-%s_EMEnriched_TuneCUETP8M1_13TeV_pythia8_76X_MiniAOD_histograms.root",iMass);
