#define MCFakeRateClosureTest2_cxx
#include "MCFakeRateClosureTest2.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
void MCFakeRateClosureTest2Base::Loop() {};

class MCFakeRateClosureTest2 : public MCFakeRateClosureTest2Base{

public:
  using MCFakeRateClosureTest2Base::MCFakeRateClosureTest2Base;
  void Loop() {};
  void Loop(int iCut = 0, TString era="UNKNOWN", TString dataset="UNKNOWN", int pvCutLow = 0, int pvCutHigh = 500);
};

void MCFakeRateClosureTest2::Loop(int iCut, TString era, TString dataset, int pvCutLow, int pvCutHigh)
{
    std::string pvLow = std::to_string(pvCutLow);
    std::string pvHigh = std::to_string(pvCutHigh);
    ...
    std::vector<double> ptBinArray({ 50., 70., 90., 110., 130., 150., 200., 250., 300., 600.});
    ...
    // // pt spectrum of denominator objects
    // // variable binned
    // TH1D phoPtEB_denominator_varbin = new TH1D("phoPtEB_denominator_varbin","",nBins-1,ptBinArray);
    // TH1D phoPtEE_denominator_varbin = new TH1D("phoPtEE_denominator_varbin","",nBins-1,ptBinArray);
    ...
    // create output file
    TString outName = TString::Format("%s_fakerate_%s_pV%d-%d_newDenomDef.root", dataset.Data(), era.Data(), pvCutLow, pvCutHigh);
...
// double MCFakeRateClosureTest2::FakeRateEB(double pt) {
...
// };
//
// double MCFakeRateClosureTest2::FakeRateEE(double pt) {
