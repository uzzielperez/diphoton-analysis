...
int plot_kinematics(TFile *f_all, TFile *f_fakes, TString name, TString era = "2016") {
  ...
  l_EB->SetFillColor(0);
  l_EB->AddEntry(h_unweighted_EB,"Denominator","ep"); // black
  l_EB->AddEntry(h_fakes_EB,"MC truth","ep"); // red
  l_EB->AddEntry(h_all_EB,"Fake prediction","ep"); // blue
  ...
  TH1D *h_ratio_EB = (TH1D*) h_fakes_EB->Clone("h_ratio_EB");
  ...
  if (name == "pt") h_ratio_EB->SetMaximum(0.1);
  h_ratio_EB->SetStats(0);
  h_ratio_EB->Divide(h_unweighted_EB);
  //h_ratio_EB->Divide(h_unweighted_EB);
  ...
  TH1D *h_ratio_EE = (TH1D*) h_fakes_EE->Clone("h_ratio_EE");
  h_ratio_EE->SetLineColor(kBlack);
  h_ratio_EE->SetMarkerColor(kBlack);
  h_ratio_EE->SetMinimum(0.4);
  h_ratio_EE->SetMaximum(1.3);
  if (name == "pt") h_ratio_EE->SetMinimum(0);
  if (name == "pt") h_ratio_EE->SetMaximum(1.0);
  h_ratio_EE->SetStats(0);
  //h_ratio_EE->Divide(h_unweighted_EE);
  h_ratio_EE->Divide(h_unweighted_EE);
  ...
  c->SaveAs("debug_closure_test_photon_kinematics_"+name+"_"+era+".pdf");
...
void debug_plot_kinematics_with_ratio(TString sample =  "all", TString era= "2016") {
