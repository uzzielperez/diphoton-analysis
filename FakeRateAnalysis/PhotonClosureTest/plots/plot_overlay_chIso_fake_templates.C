...
void plot_overlay_chIso_fake_templates() {
  ...
  for (int i = 0; i < nBins-1; ++i) {
    ...
    // EB - linear
    c->cd(1);
    TH1D *h_EB = (TH1D*) f->Get("chIsoEB_faketemplate_pt"+bin+"_sieie0.0105To1.0000");
    ...
    h_EB->SetTitle("EB - 0.0105 < #sigma_{i#etai#eta} < 1.0000");
    ...
    // EE
    c->cd(2);
    TH1D *h_EE = (TH1D*) f->Get("chIsoEE_faketemplate_pt"+bin+"_sieie0.0280To1.0000");
    ...
    h_EE->SetTitle("EE - 0.0280 < #sigma_{i#etai#eta} < 1.0000");
  ...
  c->SaveAs("fake_templates_chIso_overlaid_sample_"+sample+".pdf");
